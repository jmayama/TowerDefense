...
void OpenGLRenderer::drawCircle(float aCenterX, float aCenterY, float aRadius, bool aIsFilled, int aLineSegments)
{
	...
	{
		vertices.push_back((float)(aCenterX + (cosf((float)(M_PI * i / 180.0)) * aRadius)));
		vertices.push_back((float)(aCenterY + (sinf((float)(M_PI * i / 180.0)) * aRadius)));
	}
...
void OpenGLRenderer::drawTexture(OpenGLTexture* aTexture, float aX, float aY, float aAngle)
{
    if(aTexture != NULL)
    {
        drawTexture(aTexture, aX, aY, (float)(aTexture->getSourceWidth()), (float)(aTexture->getSourceHeight()), aAngle);
    }
}
