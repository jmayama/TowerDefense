		...
		case WM_MOUSEMOVE:
			{
				...
				if(m_LastMouseX != -1 && m_LastMouseY != -1)
				{
					...
					InputManager::getInstance()->handleMouseMovement((float)differenceX, (float)differenceY, (float)positionX, (float)positionY);
				}
		...
		case WM_LBUTTONDOWN:
			{
				int positionX = GET_X_LPARAM(aLParam);
				int positionY = GET_Y_LPARAM(aLParam);
				InputManager::getInstance()->handleMouseLeftClickDown((float)positionX, (float)positionY);
			}
			break;
		...
		case WM_LBUTTONUP:
			{
				int positionX = GET_X_LPARAM(aLParam);
				int positionY = GET_Y_LPARAM(aLParam);
				InputManager::getInstance()->handleMouseLeftClickUp((float)positionX, (float)positionY);
			}
			break;
		...
		case WM_RBUTTONDOWN:
			{
				int positionX = GET_X_LPARAM(aLParam);
				int positionY = GET_Y_LPARAM(aLParam);
				InputManager::getInstance()->handleMouseRightClickDown((float)positionX,(float) positionY);
			}
			break;
		...
		case WM_RBUTTONUP:
			{
				int positionX = GET_X_LPARAM(aLParam);
				int positionY = GET_Y_LPARAM(aLParam);
				InputManager::getInstance()->handleMouseRightClickUp((float)positionX, (float)positionY);
			}
            break;
    }
}
...
void OpenGLWindow::resize(GLsizei aWidth, GLsizei aHeight)
{
	...
	{
		//
		ScreenManager::getInstance()->setScreenSize((float)m_WindowWidth,(float) m_WindowHeight);
